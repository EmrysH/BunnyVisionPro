...
class SingleRobotTeleopNode(BimanualMonitorNode):
    ...
    def apply_teleop_init_config(self, init_config: InitializationConfig):
        ...
        left_base_mat = pt.transform_from_pq(self.robot_base_pose[1])
        ...
        if self.use_web_viz:
            ...
            self.left_robot_viz.init_robot_base_pose(self.robot_base_pose[1])
            ...
            if self.bimanual_alignment_mode == BimanualAlignmentMode.ALIGN_CENTER:
                ...
                self.left_robot_viz.create_init_frame(global_init_pose[1])
            ...
            self.left_robot_viz.update_robot(init_config.init_qpos[1])
    ...
    def update_last_retargeted_qpos(self):
        ...
        left_qpos = self.left_hand_arm.update_last_retargeted_qpos(joints[1])
        ...
        with self.client_lock:
            ...
            self.client_qpos[1][self.left_hand_arm.index_client2optimizer] = left_qpos[
                self.left_hand_arm.retargeting_joint_indices
            ]
    ...
    def publish_periodically(self):
        if self.initialized:
            with self.left_hand_arm.motion_control_lock:
                ...
                # )
                self.client_qpos[1][self.left_hand_arm.index_client2control[:7]] = (
                    self.left_hand_arm.last_control_qpos[:7]
                )
            ...
            with self.client_lock:
                ...
                self.client_ee_pose[1][:] = left_ee_target_pose
    ...
    def viz_periodically(self):
        if self.initialized:
            ...
            self.left_robot_viz.update_ee_target(ee_pose[1])
            ...
            self.left_robot_viz.update_robot(qpos[1])
    ...
    def _after_init(self):
        ...
        if self.use_web_viz:
            ...
            self.left_robot_viz.create_ee_target(ee_poses[1], self.robot_base_pose[1])
